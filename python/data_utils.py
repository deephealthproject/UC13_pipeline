...
def load_data(base_dirs = ['.'], exclude_seizures = False, verbose = 1):
    """ 
    Loads the data from some directories specified in base_dirs.
    The data has already been processed applying filter banks.
    ...
    :return X, Y
        Two numpy arrays with the data in X and the labels in Y in one-hot-encoding format.
    ...
    for base_dir in base_dirs:
        for filename in os.listdir(base_dir):
            if filename.endswith("fbank.pkl.pbz2"):
                data = decompress_pickle(base_dir + '/' + filename)
                summary_file = open(base_dir + '/' + filename[:5] + '-summary.txt', 'r')
                lines = summary_file.readlines()

                for i in range(len(lines)):
                    line = lines[i].split()
                    if len(line) > 0:
                        if line[0]=='File' and line[2]==filename[:8] + '.edf':
                            seizures = int(lines[i+3].split()[-1])
                            labels = numpy.zeros([data.shape[0], 2])
                            labels[:,0] = 1
                            #
                            if seizures != 0 and not exclude_seizures:
                                j = i + 4
                                for s in range(seizures):
                                    # Get start and end time of each seizure
                                    processed = False
                                    while (not processed):
                                        l = lines[j].split()
                                        ...
                                        if l[0]=='Seizure' and 'Start' in l:
                                            start = (int(l[-2]) - 1) // 5
                                            end = (int(lines[j+1].split()[-2]) - 1) // 5

                                            if start > 0 and end < 720:
                                                # Seizure not at the edges
                                                labels[start-1:end-1,:] = [0,1]
                ...
                if first_time:
                    ...
                    X = numpy.concatenate((X, data))
                    Y = numpy.concatenate((Y, labels))
    #
    if verbose > 0: print("Data loaded.")
